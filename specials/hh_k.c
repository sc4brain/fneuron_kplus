...
#pragma loop prefetch_sequential
  for (_iml = 0; _iml < _cntml; ++_iml) {
    _p = _ml->_data[_iml];
...
#pragma loop simd
  for (_iml = 0; _iml < _cntml; _iml++){
#pragma statement simd_list
    if (likely(0 <=_i_table[_iml] < 200)){
	;
  }else if(unlikely(_i_table[_iml] >= 200)){
	_theta_table[_iml] = 1.0; _i_table[_iml] = 199;over+=1;
    }else if(unlikely(_i_table[_iml] < 0.0)){
	_theta_table[_iml] = 0.0; _i_table[_iml] = 0;above+=1;
    }
  }
  //  printf("%lf,%lf ",(double)over/_cntml,(double)above/_cntml);
#ifdef KPLUS_USE_MOD_OMP2
#pragma omp for
#endif
#pragma loop prefetch_strong

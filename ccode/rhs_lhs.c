...
#endif
//#include "../../nrn-7.3/src/nrniv/nrniv_mf.h"
...
#pragma loop prefetch
  for (i = i2; i < i3-1; i+=2) {
    ...
    double dv1 = vec_v[i] - vec_v[i+1];
/* our connection coefficients are negative so */
    //VEC_RHS(i) -= VEC_B(i)*dv;
      //VEC_RHS(_nt->_v_parent_index[i]) += VEC_A(i)*dv;
    vec_rhs[v_parent_index[i]] += vec_a[i] *dv;
    vec_rhs[i] = vec_rhs[i] - vec_b[i]* dv + vec_a[i+1] *dv1;
    vec_rhs[i+1] = vec_rhs[i+1] - vec_b[i+1]* dv1;
...
#pragma loop prefetch
   for (i=i2; i < i3-1; i+=2) {
    ...
    vec_d[i] = vec_d[i] - vec_b[i] - vec_a[i+1];
    vec_d[i+1] = vec_d[i+1] - vec_b[i+1];

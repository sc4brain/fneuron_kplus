...
#pragma loop noalias
  ...
  //      #pragma loop prefetch_iteration 150
  // #pragma loop prefetch_stride soft
    for (i = i3 - 1; i > i2; i-=2) {
    double p,p1;
    p = vec_a[i] *(1/ vec_d[i]);   
    vec_rhs[i-1] -= p * vec_rhs[i];
    vec_d[i-1] -= p * vec_b[i];
    p1 = vec_a[i-1] *(1/ vec_d[i-1]);
    vec_rhs[v_parent_index[i-1]] -= p1 * vec_rhs[i-1];
    vec_d[v_parent_index[i-1]] -= p1 * vec_b[i-1];
...
#pragma loop prefetch_indirect
#pragma loop prefetch
#pragma statement scache_isolate_assign vec_rhs
  //#pragma loop noalias
  for (i = i2; i < i3-1; i+=2) {
    vec_rhs[i] = (vec_rhs[i] - (vec_b[i] * vec_rhs[v_parent_index[i]])) * myvec_d[i];
    vec_rhs[i+1] = (vec_rhs[i+1] - (vec_b[i+1] * vec_rhs[i])) * myvec_d[i+1];
  }

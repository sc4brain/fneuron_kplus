...
void NetParEvent::savestate_restore(double tt, NetCvode* nc){
	...
	if (use_compress_) {
		t_exchange_ = t;
		printf("use_compress_");
	}else{
	  printf("use_compress_else");}
#endif
	if (ithread_ == 0) {
...
void nrn_spike_exchange() {
  //  	if (!active_) { return; }
       	...
       	if (use_compress_)
#endif
 { nrn_spike_exchange_compressed(); return; }
	double wt;
	int i, n;
#if NRNSTAT
	nsend_ += nout_;
...
//}
	...
	for (i = 0; i < nrnmpi_numprocs; ++i) {
      ...
      //      printf("nrn_use_localgid_else");
	for (i = 0; i < nrnmpi_numprocs; ++i) {
		int j;
		int nn = nin_[i];
		if (nn > ag_send_nspike_) {//printf("netpar nn:%d,ag:%d ",nn,ag_send_nspike_);
nn = ag_send_nspike_; }
		idx = 2 + i*ag_send_size_;
		for (j=0; j < nn; ++j) {
			// order is (firetime,gid) pairs.
...
void BBS::netpar_solve(double tstop) {
	...
	tstopunset;
	if (cvode_active_) {
	  printf("cvode_active_");
		mt = 1e-9 ; md = mindelay_;
	}else{
	  //  printf("cvode_active_else");

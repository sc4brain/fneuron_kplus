...
; as user cannot get .hoc icon or association with neuron.exe
; even though one can manually associate.
...
Section "${NEURON} (required)"
  ...
  WriteRegStr HKCU "Software\NEURON\${NRN}" "Install_Dir" "$INSTDIR"
...
Section "Associate .hoc and .nrnzip"
  ...
  WriteRegStr HKCU "Software\Classes\.hoc" "" "NEURONFile"
  WriteRegStr HKCU "Software\Classes\NEURONFile" "" "NEURON model file"
  WriteRegStr HKCU "Software\Classes\NEURONFile\shell" "" "open"
  WriteRegStr HKCU "Software\Classes\NEURONFile\DefaultIcon" "" "$INSTDIR\bin\nrniv10.ico"
  WriteRegStr HKCU "Software\Classes\NEURONFile\shell\open\command" "" '"$INSTDIR\bin\neuron.exe" "%1"'
  WriteRegStr HKCU "Software\Classes\.nrnzip" "" "NEURONArchive"
  WriteRegStr HKCU "Software\Classes\NEURONArchive" "" "NEURON model archive"
  WriteRegStr HKCU "Software\Classes\NEURONArchive\shell" "" "open"
  WriteRegStr HKCU "Software\Classes\NEURONArchive\DefaultIcon" "" "$INSTDIR\bin\nrniv.ico"
  WriteRegStr HKCU "Software\Classes\NEURONArchive\shell\open\command" "" '"$INSTDIR\bin\mos2nrn.exe" "%1"'
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\NEURON\${NRN}"
  ; remove registry keys if we own them
  IfFileExists "$INSTDIR\lib\associate.dat" +1 asoc
    DeleteRegKey HKCU "Software\Classes\NEURONFile"
    DeleteRegKey HKCU "Software\Classes\NEURONArchive"
asoc:

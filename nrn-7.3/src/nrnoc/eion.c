...
static void ion_cur(NrnThread* nt, Memb_list* ml, int type)
{
  ...
  if(ion_charge==0){
    ...
    int j,k;
    for(j=0; j+BUFSIZE<count; j+=BUFSIZE){
      for (k=j,i=0; k<j+BUFSIZE; k++,i++) {
	buf[i] = pd[k][2]/pd[k][1];
      }

      for (i=0; i<BUFSIZE; i++) {
	buf[i] = _ktf * rev_ion_charge * log(buf[i]);
      }

      for (k=j,i=0; k<j+BUFSIZE; k++,i++) {
	pd[k][0] = buf[i]; pd[k][3] = 0.; pd[k][4] = 0.;
      }
    }

    for (k=j,i=0; k<count; k++,i++) {
      buf[i] = pd[k][2]/pd[k][1];
    }

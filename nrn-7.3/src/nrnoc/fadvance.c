...
void nrn_fixed_step_group(int n) {
	...
	if (nrn_multisplit_setup_) {
	  ...
	  while(step_group_end < step_group_n) {
	    //printf("step_group_end=%d step_group_n=%d thread=%d\n", step_group_end, step_group_n, nrn_nthread);

/**************************** KPLUS ***********************************/
#ifdef KPLUS_FADVANCE
	    nrn_multithread_job_omp(nrn_fixed_step_group_thread_omp);
	    if (nrn_allthread_handle) { (*nrn_allthread_handle)(); }
#else
